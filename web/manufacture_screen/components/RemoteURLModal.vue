<template>
  <TransitionRoot :show="isOpenRemoteURL">
    <Dialog :modelValue="isOpenRemoteURL" @update:modelValue="$emit('update:isOpenRemoteURL')"
      class="relative z-10 rounded-lg shadow-md">
      ...
      <div class="fixed w-full inset-0 overflow-y-auto">
        <div class="flex min-h-full items-center justify-center p-4 text-center">
          <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
            ...
            <DialogPanel
              class="w-full transform overflow-hidden rounded-2xl bg-primary-500 p-6 text-left align-middle shadow-xl transition-all">
              <!-- Modal content -->
              <select v-model="localAllowedRemoteIndex"
                class="w-full p-2 text-sm font-medium border rounded-lg shadow-sm hover:shadow-md">
              ...
              <div class="w-full flex justify-between space-x-2 pt-4">
                <button type="button" @click="$emit('update-base-url', localAllowedRemoteIndex)"
                  class="w-full bg-secondary-500 text-white font-bold py-2 px-4 rounded">
                  Aceptar
                </button>
                <button type="button" @click="$emit('update:isOpenRemoteURL', false)"
                  class="w-full bg-secondary text-old-copper-900 font-bold py-2 px-4 rounded">
                  Cancelar
                </button>
              </div>
            </DialogPanel>
